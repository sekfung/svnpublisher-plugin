        ...
        try {
            ...
            if (stmt.next()) {
                ...
                if (info.hasField(NodeInfo.revision)) {
                    info.set(NodeInfo.revision, getColumnRevNum(stmt, NODES__Fields.revision));
                }
                if (info.hasField(NodeInfo.reposRelPath)) {
                    info.set(NodeInfo.reposRelPath, getColumnPath(stmt, NODES__Fields.repos_path));
                }
                ...
                if (info.hasField(NodeInfo.reposRootUrl) || info.hasField(NodeInfo.reposUuid)) {
                    if (isColumnNull(stmt, NODES__Fields.repos_id)) {
                        ...
                        Structure<RepositoryInfo> repositoryInfo = wcroot.getDb().fetchRepositoryInfo(wcroot.getSDb(), getColumnInt64(stmt, NODES__Fields.repos_id));
                        repositoryInfo.
                ...
                if (info.hasField(NodeInfo.changedRev)) {
                    info.set(NodeInfo.changedRev, getColumnRevNum(stmt, NODES__Fields.changed_revision));
                }
                if (info.hasField(NodeInfo.changedDate)) {
                    info.set(NodeInfo.changedDate, getColumnDate(stmt, NODES__Fields.changed_date));
                }
                if (info.hasField(NodeInfo.changedAuthor)) {
                    /* Result may be NULL. */
                    info.set(NodeInfo.changedAuthor,getColumnText(stmt, NODES__Fields.changed_author));
                }
                ...
                if (info.hasField(NodeInfo.checksum)) {
                    if (node_kind != SVNWCDbKind.File) {
                        ...
                        try {
                            info.set(NodeInfo.checksum, getColumnChecksum(stmt, NODES__Fields.checksum));
                ...
                if (info.hasField(NodeInfo.target)) {
                    if (node_kind != SVNWCDbKind.Symlink)
                        ...
                        info.set(NodeInfo.target, getColumnPath(stmt, NODES__Fields.symlink_target));
                }
                if (info.hasField(NodeInfo.updateRoot)) {
                    info.set(NodeInfo.updateRoot, getColumnText(stmt, NODES__Fields.file_external) != null);
                }
    ...
    protected static Structure<NodeInfo> readInfo(SVNWCDbRoot wcRoot, File localRelPath, boolean isAdditionMode, NodeInfo... fields) throws SVNException {
        ...
        try {
            ...
            if (haveInfo) {
                ...
                info.set(NodeInfo.reposId, opDepth != 0 ? SVNWCDb.INVALID_REPOS_ID : getColumnInt64(stmtInfo, NODES__Fields.repos_id));
                if (info.hasField(NodeInfo.revision)) {
                    info.set(NodeInfo.revision, opDepth != 0 ? SVNWCDb.INVALID_REVNUM : getColumnRevNum(stmtInfo, NODES__Fields.revision));
                }
                if (info.hasField(NodeInfo.reposRelPath)) {
                    info.set(NodeInfo.reposRelPath, opDepth != 0 ? null : SVNFileUtil.createFilePath(getColumnText(stmtInfo, NODES__Fields.repos_path)));
                }
                if (info.hasField(NodeInfo.changedDate)) {
                    info.set(NodeInfo.changedDate, SVNWCUtils.readDate(getColumnInt64(stmtInfo, NODES__Fields.changed_date)));
                }
                if (info.hasField(NodeInfo.changedRev)) {
                    info.set(NodeInfo.changedRev, getColumnRevNum(stmtInfo, NODES__Fields.changed_revision));
                }
                if (info.hasField(NodeInfo.changedAuthor)) {
                    info.set(NodeInfo.changedAuthor, getColumnText(stmtInfo, NODES__Fields.changed_author));
                }
                if (info.hasField(NodeInfo.recordedTime)) {
                    info.set(NodeInfo.recordedTime, getColumnInt64(stmtInfo, NODES__Fields.last_mod_time));
                }
                if (info.hasField(NodeInfo.depth)) {
                    if (nodeKind != SVNWCDbKind.Dir) {
                        info.set(NodeInfo.depth, SVNDepth.UNKNOWN);
                    } else {
                        info.set(NodeInfo.depth, getColumnDepth(stmtInfo, NODES__Fields.depth));
                    }
                }
                if (info.hasField(NodeInfo.checksum)) {
                    if (nodeKind != SVNWCDbKind.File) {
                        ...
                        try {
                            info.set(NodeInfo.checksum, getColumnChecksum(stmtInfo, NODES__Fields.checksum));
                        } catch (SVNException e) {
                            SVNErrorMessage err = SVNErrorMessage.create(e.getErrorMessage().getErrorCode(), "The node ''{0}'' has a corrupt checksum value.", wcRoot.getAbsPath(localRelPath));
                            SVNErrorManager.error(err, SVNLogType.WC);
                        }
                ...
                if (info.hasField(NodeInfo.recordedSize)) {
                    info.set(NodeInfo.recordedSize, getColumnInt64(stmtInfo, NODES__Fields.translated_size));
                }
                if (info.hasField(NodeInfo.target)) {
                    info.set(NodeInfo.target, SVNFileUtil.createFilePath(getColumnText(stmtInfo, NODES__Fields.symlink_target)));
                }
                ...
                info.set(NodeInfo.originalReposId, opDepth == 0 ? SVNWCDb.INVALID_REPOS_ID : getColumnInt64(stmtInfo, NODES__Fields.repos_id));
                if (info.hasField(NodeInfo.originalRevision)) {
                    info.set(NodeInfo.originalRevision, opDepth == 0 ? SVNWCDb.INVALID_REVNUM : getColumnRevNum(stmtInfo, NODES__Fields.revision));
                }
                if (info.hasField(NodeInfo.originalReposRelpath)) {
                    info.set(NodeInfo.originalReposRelpath, opDepth == 0 ? null : SVNFileUtil.createFilePath(getColumnText(stmtInfo, NODES__Fields.repos_path)));
                }
