    ...
    public static long[] getMinAndMaxRevisions(SVNWCDb db, File localAbsPath) throws SVNException {
        ...
        SVNSqlJetSelectStatement stmt = new SVNSqlJetSelectStatement(sdb, SVNWCDbSchema.NODES) {
            ...
            protected boolean isFilterPassed() throws SVNException {
                ...
                String path = getColumnString(NODES__Fields.local_relpath);
                if ("".equals(localRelpath.getPath()) || path.equals(localRelpath.getPath()) || path.startsWith(localRelpath.getPath() + "/")) {
                    long depth = getColumnLong(NODES__Fields.op_depth);
                    if (depth != 0) {
                        return false;
                    }
                    ...
                    long rev = getColumnLong(NODES__Fields.revision);
                    long changedRev = getColumnLong(NODES__Fields.revision);
                    if (getColumnBoolean(NODES__Fields.file_external)) {
                        return false;
                    }
    ...
    public static boolean hasSwitchedSubtrees(SVNWCDb db, File localAbspath) throws SVNException {
        ...
        try {
            ...
            if ("".equals(localRelPathStr)) {
                if (!cursor.eof()) {
                    parentReposRelpath = cursor.getString(NODES__Fields.repos_path.toString());
            ...
            while(!cursor.eof()) {
                String rowRelPath = cursor.getString(NODES__Fields.local_relpath.toString());
                boolean fileExternal = cursor.getBoolean(NODES__Fields.file_external.toString());
                if (fileExternal) {
                } else if (rowRelPath.equals(parentRelPath)) {
                    long opDepth = cursor.getInteger(NODES__Fields.op_depth.toString());
                    if (opDepth == 0) {
                        parents.put(rowRelPath, cursor.getString(NODES__Fields.repos_path.toString()));
                    }
                } else if ("".equals(localRelPathStr) || rowRelPath.startsWith(localRelPathStr + "/")) {
                    matched = true;
                    long opDepth = cursor.getInteger(NODES__Fields.op_depth.toString());
                    if (opDepth == 0) {
                        String rowReposRelpath = cursor.getString(NODES__Fields.repos_path.toString());
                        String rowParentRelpath = cursor.getString(NODES__Fields.parent_relpath.toString());
                        if ("dir".equals(cursor.getString(NODES__Fields.kind.toString()))) {
    ...
    public static boolean hasLocalModifications(SVNWCContext context, File localAbspath) throws SVNException {
        ...
        try {
            ...
            while(!cursor.eof()) {
                ...
                if ("".equals(localRelPathStr) || rowRelPath.equals(localRelPathStr) || rowRelPath.startsWith(localRelPathStr + "/")) {
                    matched = true;
                    String kind = cursor.getString(NODES__Fields.kind.toString());
                    if ("file".equals(kind)) {
                        String presence = cursor.getString(NODES__Fields.presence.toString());
                        if ("normal".equals(presence) && !cursor.getBoolean(NODES__Fields.file_external.toString())) {
                            ...
                            long size = cursor.getInteger(NODES__Fields.translated_size.toString());
                            long date = cursor.getInteger(NODES__Fields.last_mod_time.toString());
                            if (size != -1 && date != 0) {
        ...
        try {
            ...
            while(!cursor.eof()) {
                ...
                if (fileExternal) {
                    ...
                    long opDepth = cursor.getInteger(NODES__Fields.op_depth.toString());
                    if (opDepth == 0) {
                        SVNWCDbStatus presence = SvnWcDbStatementUtil.parsePresence(cursor.getString(NODES__Fields.presence.toString()));
                        if (presence == SVNWCDbStatus.Excluded || presence == SVNWCDbStatus.ServerExcluded) {
                            return true;
                        }
                        SVNDepth depth = SvnWcDbStatementUtil.parseDepth(cursor.getString(NODES__Fields.depth.toString()));
